...
void CourseDialog::validateInput()
{
    ...
    // 检查节次是否在有效范围内
    int maxSection = ScheduleManager::getSectionTimes().size();
    if (start < 1 || start > maxSection || end < 1 || end > maxSection) {
        QMessageBox::warning(this, "警告", QString("节次必须在1-%1之间").arg(maxSection));
        return;
    }
