...
bool Settings::isTrayEnabled() const
{
    return m_settings->value("Interface/TrayEnabled", DEFAULT_TRAY_ENABLED).toBool();
}

void Settings::setTrayEnabled(bool enabled)
{
    if (enabled != isTrayEnabled()) {
        m_settings->setValue("Interface/TrayEnabled", enabled);
